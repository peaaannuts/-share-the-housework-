...
- 音声ファイルは**任意・部分実装OK**（TIPSイラストの `image` と同じ「あれば
  差し替わる」設計）。`public/voices/` に生成できたものから少しずつ置ける。
  ファイルが無い/読み込めない場合は `useIemoriVoice.ts` が黙って無視し、
  テキスト切り替えは今まで通り動く。
