...
5. **長押しの直後に来る click を飲み込む**
   長押しで何かをその場に出す（`QuickTimeSheet`）と、指を離したときの
   `click` が**今まさに指の下に現れた要素**に当たる。ホームのお気に入り行で
   これが起き、開いたシートのプリセットボタン（「さっき」）を勝手に押して
   記録＋即クローズ → iPhoneからは「長押しが効かない」に見えていた。
   `useLongPress` が pointerup 時に capture フェーズで次の click を1回だけ
   握りつぶすことで解決している（`swallowNextClick`）。
   併せて、自前の長押しを持つ行には `.long-pressable`（`index.css`）を付ける。
   素の `<div>` だと iOS Safari が自前のテキスト選択・コールアウトの
   長押しジェスチャを始めて press を奪う。`<button>` は既定で除外されるため、
   「すべての家事」シートの行だけ動いて見えたのはこれが理由。
