...
## いえもりの音声（ComfyUIで生成、21種類すべて実装済み）

「タップでもうひとこと」で台詞が切り替わると同時に、対応する音声
（事前収録・ComfyUIで生成）を再生する。`iemoriLines()`（`HomeTab.tsx`）が
返す各台詞は `{ key, text, hasVoice }` を持ち、`key` が音声ファイル名
`public/voices/{key}.mp3` に対応する。

- **声のディレクション: 「イケてるおじさん」風**（ユーザー指定）。台詞自体が
  「〜ですねぇ」「〜まいりましょう」「〜よろしい」という穏やかな年長者口調
  なので、渋め・低め・落ち着いた中年男性ボイスの方向でComfyUI側のモデル/
  プロンプトを選ぶとテイストが合う。
- `hasVoice: false` の行（`remain`/`thanksToday`/`selfMore`/`partnerMore` の
  4つ）は、台詞に可変値（残数・🍀合計・ニックネーム）が埋め込まれていて
  事前収録の音声を当てられないので、テキストのみ（無音）のまま。時間帯
  分岐（`morning`/`afternoon`/`evening`）のように**選ばれ方は動的でも結果の
  文言自体が固定**の行は `hasVoice: true` で音声を持てる——20個の表示スロット
  から実際に生成しうる音声ファイルは**21種類**（`allDone`/`together`/
  `noRush`/`evenHands`/`morning`/`afternoon`/`evening`/その他14個の固定行）。
- 音声ファイルは**任意**（TIPSイラストの `image` と同じ「あれば差し替わる」
  設計）で、ファイルが無い/読み込めない場合は `useIemoriVoice.ts` が黙って
  無視しテキスト切り替えは今まで通り動く——ただし現状は`hasVoice: true`の
  **21種類すべて `public/voices/{key}.mp3` に揃っている**（ComfyUI生成、
  MP3・モノラル・48kHz・64kbps）。追加/差し替えも同じ場所に同名で置くだけ。
- 形式・サイズの目安: TIPSイラストの「WebP・長辺800px・40〜60KB」に相当する
  音声側の目安として、**MP3（Safari/iOS PWA互換のため）・低ビットレート
  （32〜48kbps・モノラル）・1本あたり数秒〜十数秒で数十KB程度**。
- ミュート設定（🔊/🔇、いえもりアイコン右上の小さいボタン）は世帯間で
  同期するFirestoreのデータではなく、**この端末で鳴らすかどうか**なので
  `localStorage`（`iemoriVoiceMuted`）に device-local で持つ——このアプリで
  `localStorage` を使う最初の箇所。
...
`public/voices/*.mp3` をリポジトリに追加してコミットするだけでよい。
アプリのコード（GitHubリポジトリ）と、世帯・家事記録などのデータ
（Firestore）は完全に別のシステムなので、混同しないこと。
